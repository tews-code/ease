...
macro_rules! define_buddy {
    ($mod_name:ident, $min_block_size:literal, $max_order:literal) => {
        pub mod $mod_name {
            ...
            pub const BITS: usize = (1 << $max_order) - 1; // 32767
            const ORDERS_COUNT: usize = $max_order + 1; //  Range from 8B to 256KiB
            ...
            impl BuddyInner {
                ...
                pub unsafe fn pop(&mut self, order: usize) -> *mut FreeBlock {
                    ...
                    if order < $max_order {
                        let bit = Buddy::pair_bit(order, block.addr() - self.heap_addr);
            ...
            impl Buddy {
                ...
                pub const fn order_bit_offset(order: usize) -> usize {
                ...
                pub const fn pair_bit(order: usize, offset: usize) -> usize {
                    ...
                    let block_index =
                        offset >> (($min_block_size as usize).ilog2() as usize + order);
                    let pair_index = block_index >> 1;
                    Self::order_bit_offset(order) + pair_index
                }
            ...
            unsafe impl GlobalAlloc for Buddy {
                unsafe fn alloc(&self, layout: core::alloc::Layout) -> *mut u8 {
                    ...
                    while current_order <= inner.top_order
                        && inner.list_heads[current_order].is_null()
                    {
                        current_order += 1;
                    }
                    ...
                    #[cfg(test)]
                    let _ = PADDING_BYTES
                        .fetch_add((alloc_size - layout.size()) as u32, Ordering::Relaxed);
                    #[cfg(test)]
            ...
            mod host_tests {
                ...
                fn order_bit_offset_known_values() {
                    ...
                    assert_eq!(Buddy::order_bit_offset(0), 0);
                    assert_eq!(Buddy::order_bit_offset(1), 1 << ($max_order - 1));
                    assert_eq!(
                        Buddy::order_bit_offset(2),
                        (1 << $max_order) - (1 << ($max_order - 2))
                    );
                    // order MAX_ORDER-1 is the last in-range entry; equals BITS - 1.
                    assert_eq!(Buddy::order_bit_offset($max_order - 1), BITS - 1);
                    // MAX_ORDER is one past the last valid order — equals BITS.
                    // Documented here rather than encouraged via pair_bit.
                    assert_eq!(Buddy::order_bit_offset($max_order), BITS);
                }

                #[test]
                fn pair_bit_concrete_values() {
                    // Order 0 blocks are MIN_BLOCK_SIZE; consecutive pairs sit at offsets
                    // [0, MIN], [2*MIN, 3*MIN], [4*MIN, 5*MIN], ...
                    assert_eq!(Buddy::pair_bit(0, 0), 0);
                    assert_eq!(Buddy::pair_bit(0, $min_block_size), 0);
                    assert_eq!(Buddy::pair_bit(0, 2 * $min_block_size), 1);
                    assert_eq!(Buddy::pair_bit(0, 3 * $min_block_size), 1);
                    assert_eq!(Buddy::pair_bit(0, 4 * $min_block_size), 2);
                    // Order 1 blocks are 2 * MIN_BLOCK_SIZE; order-1 bits start at
                    // order_bit_offset(1) = 2^(MAX_ORDER-1).
                    assert_eq!(Buddy::pair_bit(1, 0), 1 << ($max_order - 1));
                    assert_eq!(
                        Buddy::pair_bit(1, 2 * $min_block_size),
                        1 << ($max_order - 1)
                    );
                    assert_eq!(
                        Buddy::pair_bit(1, 4 * $min_block_size),
                        (1 << ($max_order - 1)) + 1
                    );
                }
                ...
                fn pair_bit_buddies_share_bit() {
                    ...
                    for order in 0..$max_order {
                        ...
                        for pair in 0..4 {
                            ...
                            assert_eq!(
                                Buddy::pair_bit(order, left),
                                Buddy::pair_bit(order, right),
                ...
                fn pair_bit_adjacent_pairs_are_adjacent_bits() {
                    ...
                    for order in 0..$max_order {
                        ...
                        let b0 = Buddy::pair_bit(order, 0);
                        let b1 = Buddy::pair_bit(order, pair_size);
                        assert_eq!(
                            b1,
                            b0 + 1,
                ...
                fn pair_bit_order_ranges_do_not_overlap() {
                    ...
                    for order in 0..($max_order - 1) {
                        ...
                        let last_bit = Buddy::pair_bit(order, last_pair_offset);
                        let next_start = Buddy::order_bit_offset(order + 1);
                        assert!(
                            last_bit < next_start,
                ...
                fn buddy_offset_is_involution() {
                    ...
                    for order in 0..$max_order {
                        ...
                        for n in 0..8 {
                            let a = n * block_size;
                            let b = Buddy::buddy_offset(order, a);
                            assert_eq!(
                                Buddy::buddy_offset(order, b),
                                a,
                ...
                fn buddy_offset_differs_by_block_size() {
                    ...
                    for order in 0..$max_order {
                        ...
                        for n in 0..4 {
                            let a = n * block_size;
                            assert_eq!(Buddy::buddy_offset(order, a) ^ a, block_size);
                        }
                    }
                }
                ...
                fn buddies_share_pair_bit_via_both_helpers() {
                    ...
                    for order in 0..$max_order {
                        ...
                        for n in 0..8 {
                            ...
                            let b = Buddy::buddy_offset(order, a);
                            assert_eq!(
                                Buddy::pair_bit(order, a),
                                Buddy::pair_bit(order, b),
                ...
                fn basic_alloc_dealloc() {
                    let heap = TestHeap::new($min_block_size << 9);
                ...
                fn alloc_until_oom_then_recover() {
                    let heap = TestHeap::new($min_block_size << 7);
                    let a = make_allocator(&heap);
                    let layout = Layout::from_size_align(64, 8).unwrap();
...
pub use pd0::Buddy as BuddyPd0;
pub use pd1::Buddy as BuddyPd1;
#[allow(unused_imports)]
pub use psram::Buddy as BuddyPsram;
