...
pub unsafe fn init_for_user_entry(
    ...
    debug_assert!(
        stack.size() > core::mem::size_of::<Context>() + core::mem::size_of::<TrapFrame>(),
        "user stack memory region too small for context switch and trap return"
    );
    ...
    unsafe {
        ...
        let trap_frame_ptr = stack
            .base()
            .as_ptr()
            .add(stack.size() - core::mem::size_of::<TrapFrame>())
            as *mut TrapFrame;
        ...
        (*trap_frame_ptr).user_sp = stack.top().addr().into();
    }
    ...
    let context_ptr =
        unsafe {
            stack.base().as_ptr().add(
                stack.size() - core::mem::size_of::<TrapFrame>() - core::mem::size_of::<Context>(),
            ) as *mut Context
        };
    unsafe {
